/**
 * KRI CV draw overlay (h) — same layout as CV leader (x), one instance per CV slot.
 * Crew text is fixed at show; each card + connector tracks its boat at 45° up-right.
 */
(function (global) {
    const INTRO_MS = 2500;
    const OUTRO_MS = 2500;
    ...
    function measureLineLength(cardEl) {
        const h = cardEl?.offsetHeight || 0;
        return h > 0 ? Math.max(80, h * 2) : DEFAULT_LINE_LENGTH;
    }

    function boxAnchor(inst, ax, ay) {
        const dist = inst.lineLength * BOX_DISTANCE_FACTOR;
        return {
            bx: ax + dist * COS45,
            by: ay - dist * SIN45,
        };
    ...
    function connectorEnd(inst, ax, ay, bx, by) {
        const { ux, uy } = lineUnit(ax, ay, bx, by);
        let extend = 40;
        const card = inst.cardEl;
        if (card) {
            const panel = card.querySelector('.vg-kri-leader-panel');
            const w = (panel || card).offsetWidth || card.offsetWidth || 180;
    ...
    function connectorPoints(inst, ax, ay, bx, by) {
        const { ux, uy } = lineUnit(ax, ay, bx, by);
        const px = -uy;
        const py = ux;
        const { ex, ey } = connectorEnd(inst, ax, ay, bx, by);
        return [
    ...
    function positionCard(inst, bx, by) {
        inst.cardWrap.style.left = `${bx}px`;
        inst.cardWrap.style.top = `${by}px`;
    }

    function applyAnchor(inst, ax, ay) {
        const { bx, by } = boxAnchor(inst, ax, ay);
        inst.connector.setAttribute('points', connectorPoints(inst, ax, ay, bx, by));
        positionCard(inst, bx, by);
    ...
    function mountInstances(race, count, boatDirection) {
        ...
        for (let slot = 1; slot <= count; slot++) {
            ...
            const lineLength = measureLineLength(cardEl);
    ...
    function smoothFrame(ts) {
        ...
        for (const inst of instances) {
            ...
            applyAnchor(inst, inst.ax, inst.ay);
        }
    ...
    async function show(opts = {}) {
        ...
        for (const inst of instances) {
            const ax = OUT_W * 0.35;
            const ay = OUT_H * (0.35 + (inst.slot - 1) * 0.04);
            inst.ax = ax;
            inst.ay = ay;
            inst.targetAx = ax;
            inst.targetAy = ay;
