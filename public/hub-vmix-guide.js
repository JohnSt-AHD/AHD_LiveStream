...
const VMIX_TRIGGERS = [
    ...
    { key: 'w', graphic: 'Leader', desc: 'Milford only — leader video + hub lane text (fade at 1s); pause at 5s; 1–8 switch lane; o out' },
...
function hubRenderVmixGuide() {
    ...
    if (examples) {
        ...
        leaderNote.innerHTML =
            '<strong>Leader (<code>w</code>):</strong> Leader shot top-right at half size (GT layout). Set <strong>Leader lane</strong> on the hub (default 4); text fades in 1s after <code>w</code>. Video pauses at 5s; <code>o</code> fades text and finishes video. Press <code>1</code>–<code>8</code> on air to switch lane instantly.';
