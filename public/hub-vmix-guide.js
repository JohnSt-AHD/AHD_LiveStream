...
const VMIX_TRIGGERS = [
    ...
    { key: 'w', graphic: 'Leader', desc: 'Milford only — leader video (top-right, half size); shows hub leader lane on air; 1–8 switch lane' },
...
function hubRenderVmixGuide() {
    ...
    if (examples) {
        ...
        leaderNote.innerHTML =
            '<strong>Leader (<code>w</code>):</strong> Leader shot top-right at half size (GT layout). Set <strong>Leader lane</strong> on the hub (default 4); <code>w</code> shows that lane automatically. Press <code>1</code>–<code>8</code> on air to switch lane. Logo fades in after 1s.';
