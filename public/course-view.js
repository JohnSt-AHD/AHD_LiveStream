...
(function () {
  ...
  function scheduleCourseMapResize() {
    if (!courseMap) return;
    [0, 80, 200, 450].forEach((ms) => {
      setTimeout(() => courseMap?.invalidateSize(), ms);
    });
  }

  function setCoursePanel(panel) {
    const body = document.querySelector('.course-view__body');
    if (!body) return;
    const next = panel || 'map';
    body.dataset.activePanel = next;
    document.querySelectorAll('.course-view__tab').forEach((btn) => {
      const active = btn.getAttribute('data-course-tab') === next;
      btn.classList.toggle('is-active', active);
      btn.setAttribute('aria-selected', active ? 'true' : 'false');
    });
    if (next === 'map' || next === 'chart') {
      scheduleCourseMapResize();
    }
    if (next === 'chart') {
      const course = getDisplayCourse();
  ...
  function bindMobileTabs() {
    document.querySelectorAll('.course-view__tab[data-course-tab]').forEach((btn) => {
      btn.addEventListener('click', () => {
        setCoursePanel(btn.getAttribute('data-course-tab'));
      });
    });
  }
